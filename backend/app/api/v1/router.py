...
from app.application.chat import chat as ai_chat
...
def list_contracts(
    ...
    if contract_ids:
        ...
        files_sub = (
            select(
                ContractFile.contract_id,
                func.min(ContractFile.id).label("file_id"),
            )
            .where(ContractFile.contract_id.in_(contract_ids))
            .group_by(ContractFile.contract_id)
        )
        for fid, ffid in db.execute(files_sub).all():
            file_map[fid] = ffid
    return result(
        {
            "items": [
                {
                    ...
                    "contractFileId": file_map.get(x.id),
                    "ownerId": x.owner_id,
...
def list_reviews(
    ...
    if contract_ids:
        contract_name_map = dict(
            db.execute(
                select(Contract.id, Contract.name).where(Contract.id.in_(contract_ids))
            ).all()
        )
    return result(
        {
            "items": [
                {
                    ...
                    "contractFileId": row.contract_file_id,
                    "contractName": contract_name_map.get(row.contract_id, ""),
...
def get_review(
    ...
    feedback_rows = db.scalars(
        select(ReviewFeedback).where(ReviewFeedback.review_id == review.id)
    ).all()
    feedback_data = [
        {
...
def list_warnings(
    ...
    # 基础查询：按角色范围限定
    if user.role == "user":
        query = select(RiskWarning).where(RiskWarning.owner_id == user.id)
        if warning_status is None:
            # 默认只展示用户可见的有效预警
            query = query.where(RiskWarning.warning_status.in_({"active", "processing"}))
        elif warning_status:
            query = query.where(RiskWarning.warning_status == warning_status)
    elif user.role == "admin":
        query = select(RiskWarning)
        if warning_status:
            query = query.where(RiskWarning.warning_status == warning_status)
    else:
    ...
    data: dict[str, object] = {"items": [warning_payload(db, row, private) for row in rows], "total": total}
    if overdue:
        within_seven = db.scalar(
            select(func.count()).select_from(
                query.where(RiskWarning.due_at >= overdue_now - timedelta(days=7)).subquery()
            )
        ) or 0
        data["overdueSummary"] = {
            "total": total,
            "withinSevenDays": within_seven,
            "overSevenDays": total - within_seven,
        }
    return result(data)


@router.get("/warnings/actions/recent")
def list_recent_warning_actions(
    user: CurrentUser,
    db: Db,
    limit: int = Query(5, ge=1, le=20),
) -> dict[str, object]:
    """获取当前用户最近的预警处置记录（历史处置记录）"""
    actions = recent_actions_for_user(db, user.id, limit)
    return result({"items": actions})


@router.get("/warnings/stats")
def warnings_stats(user: CurrentUser, db: Db) -> dict[str, object]:
    """获取当前用户的预警统计"""
    return result(warning_stats_for_user(db, user.id))
...
def get_warning(
    ...
    # 预警所有者也可以查看处置记录
    if private or warning.owner_id == user.id:
        data["actions"] = warning_actions(db, warning.id)
    return result(data)


@router.post("/warnings/{warningId}/revise")
async def revise_warning(
    ...
    # 验证预警归属和状态
    warning = visible_warning(db, warning_id, user)
    if user.role != "user" or warning.owner_id != user.id:
        raise fail("PERMISSION_DENIED")
    ...
    # 校验合同
    contract = db.scalar(
        select(Contract).where(Contract.id == warning.contract_id, Contract.deleted_at.is_(None))
    )
    if contract is None:
        raise fail("CONTRACT_NOT_FOUND")
    # 检查是否有进行中的审核
    ...
    if len(content) > settings.max_upload_mb * 1024 * 1024:
        raise fail("FILE_TOO_LARGE")
    kind = file_type(file.filename or "revised", content)
    path, digest = save_upload(contract.id, file.filename or "revised", content)
    contract_file = ContractFile(
        contract_id=contract.id,
        file_name=file.filename or "revised",
        storage_path=path,
        file_type=kind,
        file_size=len(content),
        sha256=digest,
    ...
    db.flush()
    # 创建整改审核记录
    idempotency_key = f"remediation-{warning.id}-{digest}"
    previous = db.scalar(
        select(ReviewRecord).where(
            ReviewRecord.idempotency_user_id == user.id,
            ReviewRecord.idempotency_key == idempotency_key,
...
def risk_confirm(
    ...
    def go():
        ...
        if report is None:
            ...
            db.add(report)
            db.flush()
        # 整改审核完成后，更新源预警状态使其回到用户的预警中心
        complete_remediation(db, review)
        return {
            "reviewId": review.id,
            "reviewStatus": review.status,
            "reviewStage": review.review_stage,
...
def create_report(
    ...
    if user.role == "user":
        ...
        # 风控审核员只能为本人已完成复核的合同生成报告。
        if review.risk_reviewer_id != user.id:
            raise fail("PERMISSION_DENIED")
    elif user.role != "admin":
        raise fail("PERMISSION_DENIED")
...
def retry_report(
    ...
    report, review, _ = _report_access(db, report_id, user)
    if user.role != "admin" and not (
        user.role == RISK and review.risk_reviewer_id == user.id
    ):
        raise fail("PERMISSION_DENIED")

    def go() -> dict[str, object]:
        locked_report = db.scalar(select(Report).where(Report.id == report.id).with_for_update())
        if locked_report is None:
            raise fail("REPORT_NOT_FOUND")
        if locked_report.status != "failed":
            raise fail("REPORT_NOT_READY")
        if locked_report.attempt_count >= settings.report_max_attempts:
            raise fail("REPORT_GENERATION_FAILED")
        locked_report.status = "pending"
        locked_report.started_at = None
        locked_report.error_code = None
        locked_report.error_message = None
        locked_report.storage_path = None
        locked_report.file_size = None
        locked_report.sha256 = None
        locked_report.generated_at = None
        return _public_report(locked_report)
...
def preview_report(
    report_id: Annotated[int, Path(alias="reportId")],
    db: Db,
    credentials: Annotated[HTTPAuthorizationCredentials | None, Depends(bearer)] = None,
    token: str | None = Query(None, description="iframe 兼容模式：URL 传递 JWT"),
):
    """HTML 报告在线预览（支持 Bearer header 或 ?token= 查询参数）"""
    # 先尝试 Bearer header，再尝试 token 查询参数
    resolved_user: User | None = None
    if credentials is not None and credentials.scheme.lower() == "bearer":
        try:
            data = decode_token(credentials.credentials)
            resolved_user = db.get(User, int(data["sub"]))
        except AppError:
            pass
    if resolved_user is None and token:
        try:
            ...
            pass
    if resolved_user is None or not resolved_user.deleted_at and resolved_user.status != "active":
        raise fail("AUTH_TOKEN_INVALID")
    report, _, _ = _report_access(db, report_id, resolved_user)
...
# ---- AI Chat ----
@router.post("/ai/chat")
async def ai_chat_endpoint(body: ChatIn, user: CurrentUser) -> dict[str, object]:
    history = None
    if body.history:
        history = [{"role": h.role, "content": h.content} for h in body.history]
    reply = await ai_chat(body.message, history)
    return result({"reply": reply, "role": "assistant"})
