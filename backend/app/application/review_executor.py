...
SAFE_MESSAGES = {
    ...
    "AI_SERVICE_ERROR": "AI服务内部异常",
    "AI_RESPONSE_INVALID": "AI初审结果无效",
    "LLM_API_FAILED": "大模型调用失败",
    "FILE_PARSE_FAILED": "文件无法解析，请检查文件格式",
    "OCR_FAILED": "文档OCR识别失败",
    "CLASSIFICATION_FAILED": "合同分类失败",
    "ELEMENT_EXTRACTION_FAILED": "合同要素提取失败",
    "RISK_ANALYSIS_FAILED": "风险分析失败",
    "CLAUSE_COMPARISON_FAILED": "条款比对失败",
    "DATABASE_ERROR": "数据存储失败",
    "REVIEW_FAILED": "当前审核模式暂未实现",
}
AI_ERROR_CODES = {
    "AI_SERVICE_UNAVAILABLE", "AI_SERVICE_ERROR",
    "LLM_API_FAILED", "REVIEW_FAILED",
    "FILE_PARSE_FAILED", "OCR_FAILED",
    "CLASSIFICATION_FAILED", "ELEMENT_EXTRACTION_FAILED",
    "RISK_ANALYSIS_FAILED", "CLAUSE_COMPARISON_FAILED",
}
...
class ReviewExecutor:
    ...
    async def _process(self, review_id: int) -> None:
        try:
            logger.info("review_processing review_id=%s", review_id)
    ...
    def _persist(
        ...
        try:
            ...
            for risk in result.risks:
                # 数据库没规则时 rule_id 置空，避免外键约束失败
                effective_rule_id = risk.rule_id if (risk.rule_id and risk.rule_id in snapshots) else None
    ...
    def _mark_failed(self, review_id: int, code: str) -> None:
        logger.warning("review_failed review_id=%s code=%s", review_id, code)
        db = self.session_factory()
        try:
            review = db.get(ReviewRecord, review_id)
