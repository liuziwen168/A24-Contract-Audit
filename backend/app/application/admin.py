...
def dashboard_data(
    ...
    warning_filters = [
        Contract.deleted_at.is_(None),
        *_date_filters(RiskWarning.created_at, start, end),
    ]
    warning_counts = dict(
        db.execute(
            select(RiskWarning.warning_status, func.count())
            .join(Contract, Contract.id == RiskWarning.contract_id)
            .where(*warning_filters)
            .group_by(RiskWarning.warning_status)
        ).all()
    )
    dashboard_now = utcnow()
    overdue_warnings = db.scalar(
        select(func.count())
        .select_from(RiskWarning)
        .join(Contract, Contract.id == RiskWarning.contract_id)
        .where(
            *warning_filters,
            RiskWarning.warning_status.in_({"active", "processing"}),
            RiskWarning.due_at.is_not(None),
            RiskWarning.due_at < dashboard_now,
        )
    ) or 0
    report_counts = dict(
        db.execute(
            select(Report.status, func.count())
            .join(ReviewRecord, ReviewRecord.id == Report.review_id)
            .join(Contract, Contract.id == ReviewRecord.contract_id)
            .where(*review_filters)
    ...
    return {
        ...
        "warningsTotal": sum(warning_counts.values()),
        "warningsByStatus": warning_counts,
        "overdueWarnings": overdue_warnings,
        "activeUsers": active_users,
        "reportsByStatus": report_counts,
